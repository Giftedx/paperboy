...
class TestEmailSender(unittest.TestCase):
    def setUp(self):
        ...
        def side_effect(key, default=None):
            if key == ('email', 'sender'): return 'sender@example.com'
            if key == ('email', 'recipients'): return ['recipient@example.com']
            if key == ('email', 'smtp_host'): return 'smtp.example.com'
            if key == ('email', 'smtp_port'): return 587
            if key == ('email', 'smtp_user'): return 'user'
            if key == ('email', 'smtp_pass'): return 'pass'
            if key == ('email', 'smtp_tls'): return 1
            if key == ('paths', 'template_dir'): return 'templates'
            return default
    ...
    def test_send_email_no_recipients(self):
        self.mock_config_get.side_effect = lambda k, d=None: [] if k == ('email', 'recipients') else d
        date = datetime.date(2023, 10, 27)
    ...
    def test_send_alert_email_invalid_recipient(self):
        def side_effect(key, default=None):
            if key == ('email', 'alert_recipient'): return 'invalid'
            return default
        self.mock_config_get.side_effect = side_effect
    ...
    def test_thumbnail_local_file(self, mock_smtp_cls):
        ...
        with patch('email_sender._render_email_content', return_value=("Sub", "Body")):
            with patch('os.path.isfile', return_value=True):
                ...
                fake_jpg = b'\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x01\x00H\x00H\x00\x00'
                with patch('builtins.open', mock_open(read_data=fake_jpg)):
                    result = email_sender.send_email(date, "http://url", [], thumbnail_path="/path/to/thumb.jpg")

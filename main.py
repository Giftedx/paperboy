...
def update_status(step, status, message=None, percent=None, eta=None, explainer=None):
    ...
    try:
        ...
        logger.warning(
            ...
            status_file_path if "status_file_path" in locals() else STATUS_FILE,
            e,
        )
    except Exception as e:
        logger.exception(
            "Unexpected error writing status file '%s': %s",
            status_file_path if "status_file_path" in locals() else STATUS_FILE,
            e,
        )
...
def setup_configuration():
    """Loads configuration and sets up global variables and logging.

    Returns:
        bool: True if configuration loaded successfully, False otherwise.
    """
    global DATE_FORMAT, FILENAME_TEMPLATE, THUMBNAIL_FILENAME_TEMPLATE, RETENTION_DAYS, STATUS_FILE

    update_status("config_load", "in_progress", "Loading configuration...", percent=0)
    if not config.config.load():
        update_status(
            "config_load", "error", "Configuration failed. Check logs.", percent=0
        )
        return False
    update_status(
        "config_load", "success", "Configuration loaded and validated.", percent=5
    )

    # Update global constants from loaded config
    DATE_FORMAT = config.config.get(("general", "date_format"), DATE_FORMAT)
    FILENAME_TEMPLATE = config.config.get(
        ("general", "filename_template"), FILENAME_TEMPLATE
    )
    THUMBNAIL_FILENAME_TEMPLATE = config.config.get(
        ("general", "thumbnail_filename_template"), THUMBNAIL_FILENAME_TEMPLATE
    )
    ...
    configured_log_file = config.config.get(
        ("paths", "log_file"), "newspaper_emailer.log"
    )
...
def get_past_papers_from_storage(target_date: date, days: int):
    ...
    """
    logger.info(
        "Retrieving links for past %d paper(s) from cloud storage, up to %s.",
        days,
    ...
    try:
        ...
        for filename in all_files:
            try:
                ...
                if "newspaper" in filename and (
                    filename.endswith(".pdf") or filename.endswith(".html")
                ):
                    dated_files.append((file_date, filename))
            except (ValueError, IndexError):
                logger.debug(
                    "Could not parse date from filename: '%s'. Skipping.", filename
                )
                ...
                if file_date not in unique_dates_found:
                    if len(unique_dates_found) < days:
                        try:
                            ...
                            if url:
                                unique_dates_found[file_date] = (
                                    file_date.strftime(current_date_format),
                                    url,
                                )
                            else:
                                logger.warning(
                                    "Could not get URL for stored file: '%s'", filename
                                )
                        except Exception as url_e:
                            logger.exception(
                                "Error getting URL for stored file '%s': %s",
                                filename,
                                url_e,
                            )
                if len(unique_dates_found) >= days:
        ...
        past_papers_links = sorted(
            list(unique_dates_found.values()), key=lambda x: x[0], reverse=True
        )
        logger.info(
            "Collected %d past paper links from storage.", len(past_papers_links)
        )
...
def cleanup_old_files_main(target_date: date, dry_run: bool):
    ...
    logger.info(
        "Initiating cleanup of files older than %d days relative to %s.",
        RETENTION_DAYS,
        target_date.strftime(DATE_FORMAT),
    )
    current_date_format = config.config.get(("general", "date_format"), DATE_FORMAT)
    ...
    try:
        ...
        logger.info(
            "Files older than %s will be %sdeleted.",
        ...
        for filename in all_files:
            try:
                ...
                if file_date < cutoff_date:
                    action_taken = storage.delete_from_storage(
                        filename, dry_run=dry_run
                    )
                    if action_taken:  # True if successful or dry_run
                        deleted_count += 1
            except (ValueError, IndexError):
                logger.debug(
                    "Could not parse date from filename for cleanup: '%s'. Skipping.",
                    filename,
                )
...
def determine_target_date(target_date_str: str | None) -> date | None:
    """Parses the target date string or defaults to today.

    Args:
        target_date_str (str | None): The date string in DATE_FORMAT.
    ...
    if target_date_str:
        try:
            ...
            logger.critical(
                "Invalid target_date_str format: '%s'. Expected '%s'.",
                target_date_str,
                DATE_FORMAT,
            )
            update_status(
                "date_setup",
                "error",
                f"Invalid date format: {target_date_str}. Use {DATE_FORMAT}.",
                percent=10,
            )
            return None
    else:
...
def process_download(
    target_date: date, download_dir: str, dry_run: bool, force_download: bool
):
    """Downloads the newspaper file.
    ...
    Args:
        ...
        force_download (bool): Force download even if exists.
    ...
    base_save_path = os.path.join(
        ...
        FILENAME_TEMPLATE.format(
            date=target_date.strftime(DATE_FORMAT), format=""
        ).rstrip("."),
    )
    ...
    file_format = (
        newspaper_filename.split(".")[-1].lower()
        if "." in newspaper_filename
        else "unknown"
    )

    logger.info(
        "Newspaper downloaded successfully: '%s' (format: %s)",
        newspaper_path,
        file_format,
    )
    update_status(
        "download",
        "success",
        f"Newspaper downloaded: {newspaper_filename}",
        percent=40,
    )
    return True, newspaper_path


def process_upload(newspaper_path: str, newspaper_filename: str, dry_run: bool):
    """Uploads the newspaper to cloud storage.

    Args:
        newspaper_path (str): Local path to the newspaper.
        newspaper_filename (str): Name of the file in storage.
        dry_run (bool): If True, simulate upload.

    Returns:
        tuple: (success (bool), cloud_url (str))
    """
    update_status(
        "upload",
        "in_progress",
    ...
    if dry_run:
        logger.info(
            "[Dry Run] Would upload '%s' to cloud storage as '%s'.",
            newspaper_path,
            newspaper_filename,
        )
        cloud_file_url = (
            f"http://dry_run_cloud_storage_url/{newspaper_filename}"  # Placeholder
        )
        ...
        try:
            ...
            if not cloud_file_url:
                raise storage.ClientError(
                    "Failed to get cloud URL after upload (URL is None or empty)."
                )
            logger.info(
                "Successfully uploaded '%s' to cloud storage. URL: %s",
                newspaper_filename,
                cloud_file_url,
            )
            ...
            logger.exception(
                "Cloud storage upload failed for '%s': %s", newspaper_filename, e
            )
            update_status("upload", "error", f"Upload failed: {e}", percent=45)
            return False, None

    return True, cloud_file_url
...
def process_thumbnail(
    ...
    newspaper_path: str,
    file_format: str,
    newspaper_filename: str,
    dry_run: bool,
):
    """Generates and uploads a thumbnail.

    Args:
        target_date (date): The newspaper date.
        download_dir (str): Directory for files.
        newspaper_path (str): Path to the newspaper file.
        file_format (str): Format of the newspaper (pdf/html).
        newspaper_filename (str): Filename of the newspaper.
        dry_run (bool): If True, simulate operations.

    Returns:
        str | None: The cloud URL of the thumbnail, or None.
    """
    update_status(
        "thumbnail",
        "in_progress",
    ...
    thumbnail_actual_format = (
        thumbnail.THUMBNAIL_FORMAT.lower()
    )  # e.g. 'jpeg' -> 'jpg' if needed
    if thumbnail_actual_format == "jpeg":
        thumbnail_actual_format = "jpg"  # common extension

    thumbnail_output_filename = THUMBNAIL_FILENAME_TEMPLATE.format(
        date=target_date.strftime(DATE_FORMAT), format=thumbnail_actual_format
    )
    ...
    if dry_run:
        logger.info(
            "[Dry Run] Would generate thumbnail for '%s' at '%s'.",
            newspaper_path,
            thumbnail_output_path,
        )
        thumbnail_cloud_url = (
            f"http://dry_run_cloud_storage_url/{thumbnail_output_filename}"
        )
        update_status(
            "thumbnail",
            "success",
            "Thumbnail generation (simulated) complete.",
            percent=75,
        )
    else:
        if not os.path.exists(newspaper_path):
            logger.error(
                "Cannot generate thumbnail, input file '%s' does not exist.",
                newspaper_path,
            )
            update_status(
                "thumbnail", "error", "Newspaper file missing for thumbnailing.", percent=65
            )
            ...
            update_status(
                "thumbnail",
                "skipped",
                f"Unsupported format for thumbnail: {file_format}",
                percent=75,
            )
        else:
            ...
            if thumb_success and os.path.exists(thumbnail_output_path):
                logger.info(
                    "Thumbnail generated successfully: '%s'", thumbnail_output_path
                )
                try:
                    storage.upload_to_storage(
                        thumbnail_output_path, thumbnail_output_filename
                    )
                    thumbnail_cloud_url = storage.get_file_url(
                        thumbnail_output_filename
                    )
                    if not thumbnail_cloud_url:
                        raise storage.ClientError(
                            "Failed to get thumbnail cloud URL after upload."
                        )
                    ...
                    logger.exception(
                        "Failed to upload thumbnail '%s': %s",
                        thumbnail_output_filename,
                        e,
                    )
                    update_status(
                        "thumbnail", "error", "Thumbnail upload failed.", percent=75
                    )
            else:
                logger.warning(
                    "Thumbnail generation failed for '%s'. Email will be sent without a thumbnail.",
                    newspaper_filename,
                )
                update_status(
                    "thumbnail", "error", "Thumbnail generation failed.", percent=75
                )
    return thumbnail_cloud_url


def process_email(
    target_date: date,
    cloud_file_url: str | None,
    thumbnail_cloud_url: str | None,
    dry_run: bool,
):
    ...
    Args:
        target_date (date): The newspaper date.
        cloud_file_url (str | None): URL of the newspaper.
        thumbnail_cloud_url (str | None): URL of the thumbnail.
    ...
    """
    update_status(
        "email", "in_progress", "Preparing email...", percent=80, eta="approx. 30 sec"
    )
    try:
        retention_days_for_links = config.config.get(
            ("general", "retention_days_for_email_links"), 7
        )
        past_papers = (
            []
            if dry_run
            else get_past_papers_from_storage(
                target_date, days=retention_days_for_links
            )
        )
        ...
        if email_sent_or_drafted:
            ...
            update_status(
                "email", "success", f"Email {action_verb} successfully!", percent=95
            )
            return True
        else:
            update_status(
                "email", "error", "Failed to send/draft email. Check logs.", percent=80
            )
            return False
    except Exception as e:
...
def process_cleanup(target_date: date, dry_run: bool):
    """Performs cleanup of old files.

    Args:
        target_date (date): The current date.
        dry_run (bool): If True, skip actual deletion.
    """
    # Skip storage cleanup during dry runs to avoid interacting with cloud resources
    if dry_run:
        update_status(
            "cleanup", "skipped", "Cleanup skipped in dry-run mode.", percent=99
        )
    else:
        update_status(
            "cleanup",
            "in_progress",
            "Cleaning up old newspapers from cloud storage...",
            percent=97,
        )
...
def main(
    ...
    try:
        ...
        target_date = determine_target_date(target_date_str)
        if not target_date:
            return False
        ...
        success, result = process_download(
            target_date, download_dir, dry_run, force_download
        )
        if not success:
            return False
        newspaper_path = result
        newspaper_filename = os.path.basename(newspaper_path)
        file_format = (
            newspaper_filename.split(".")[-1].lower()
            if "." in newspaper_filename
        ...
        success, cloud_url = process_upload(newspaper_path, newspaper_filename, dry_run)
        if not success:
            return False
        cloud_file_url = cloud_url

        thumbnail_cloud_url = process_thumbnail(
            target_date,
            download_dir,
            newspaper_path,
            file_format,
        ...
        if not process_email(
            target_date, cloud_file_url, thumbnail_cloud_url, dry_run
        ):
            return False

        process_cleanup(target_date, dry_run)

        update_status(
            "complete", "success", "Newspaper processing complete!", percent=100
        )
        logger.info(
            "Daily newspaper processing for %s completed successfully.",
            target_date.strftime(DATE_FORMAT),
        )
        ...
        if not dry_run:
            if all(status["status"] == "ready" for status in get_last_7_days_status()):
                logger.info(
                    "Consistently successful for the past 7 days. Consider full automation if not already set up."
                )

        if dry_run and RICH_AVAILABLE:
            summary_table = Table(
                title="Dry Run Summary", show_header=True, header_style="bold magenta"
            )
            ...
            summary_table.add_row(
                "Download URL", config.config.get(("newspaper", "url"))
            )
            summary_table.add_row(
                "Cloud Upload Path",
                f"{config.config.get(('storage', 'bucket'))}/{newspaper_filename}",
            )

            recipients = config.config.get(("email", "recipients"), [])
            summary_table.add_row("Email Recipients", ", ".join(recipients))
            summary_table.add_row(
                "Thumbnail", "Simulated" if thumbnail_cloud_url else "None"
            )
            ...
            if os.path.exists(STATUS_FILE):
                ...
                if current_status_data.get("status") != "error":
                    update_status(
                        ...
                        "error",
                        final_error_msg,
                        percent=current_status_data.get("percent", 0),
                    )
            else:
                update_status(
                    "pipeline_error", "error", final_error_msg
                )  # Default percent
...
if __name__ == "__main__":
    ...
    force_download_override = (
        os.environ.get("MAIN_PY_FORCE_DOWNLOAD", "False").lower() == "true"
    )
    ...
    effective_target_date_str = (
        target_date_override
        if target_date_override
        else date.today().strftime(DATE_FORMAT)
    )

    logger.info(
        f"Running main pipeline with: Target Date='{effective_target_date_str}', Dry Run={dry_run_override}, Force Download={force_download_override}"
    )

    main_success = main(
        target_date_str=effective_target_date_str,

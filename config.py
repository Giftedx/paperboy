...
def setup_logging(log_level=logging.INFO, log_file="app.log", log_dir="logs"):
    ...
    logger.info(
        "Logging configured to level '%s', writing to '%s'.",
...
class Config:
    ...
    def __init__(self):
        ...
        self.CRITICAL_CONFIG_MAP = {
            key_path: rule for key_path, rule in CRITICAL_CONFIG_KEYS
        }
    ...
    def _get_fernet(self):
        ...
        if not CRYPTO_AVAILABLE:
            logger.warning(
                "Cryptography libraries not installed. Decryption unavailable."
            )
            return None
    ...
    def _decrypt_value(self, encrypted_value):
        ...
        if not fernet:
            logger.error(
                "Cannot decrypt value: Missing SECRETS_PASSPHRASE or SECRETS_ENC_SALT."
            )
            return None
        try:
    ...
    def validate_critical_config(self):
        ...
        for key_path, validation_rule in CRITICAL_CONFIG_KEYS:
            ...
            if value is None:
                ...
                if key_path == ("paths", "download_dir"):
                    logger.warning(
                        "Potentially missing configuration: '%s'. Application will use default or behavior might be unexpected. Define in config.yaml or as env var (%s).",
                        key_str,
                        "_".join(k.upper() for k in key_path),
                    )
                else:
                    logger.critical(
                        "MISSING critical configuration: '%s'. Please define it in config.yaml or as an environment variable (%s).",
                        key_str,
                        "_".join(k.upper() for k in key_path),
                    )
                    is_valid = False
            ...
            if validation_rule == "str":
                if not isinstance(value, str) or not value.strip():
                    logger.critical(
                        "INVALID configuration: '%s' must be a non-empty string. Found: '%s' (type: %s)",
                        key_str,
                        "********" if self._is_secret(key_path) else value,
                        type(value).__name__,
                    )
                    is_valid = False
                ...
                if not isinstance(value, int):
                    try:
                        ...
                        logger.critical(
                            "INVALID configuration: '%s' must be an integer. Found: '%s' (type: %s)",
                            key_str,
                            "********" if self._is_secret(key_path) else value,
                            type(value).__name__,
                        )
                ...
                if not isinstance(value, str) or not (
                    ...
                    logger.critical(
                        "INVALID configuration: '%s' must be a valid URL (starting with http:// or https://). Found: '%s'",
                        key_str,
                        "********" if self._is_secret(key_path) else value,
                    )
                    is_valid = False
                ...
                if isinstance(value, list):
                    if not value or not all(
                        ...
                        logger.critical(
                            "INVALID configuration: '%s' must be a non-empty list of email strings.",
                            key_str,
                        )
                        is_valid = False
                    ...
                    logger.critical(
                        "INVALID configuration: '%s' must be a list of emails or comma-separated string. Found type: %s",
                        key_str,
                        type(value).__name__,
                    )
                    is_valid = False
        ...
        if is_valid:
            ...
            logger.error(
                "Critical configuration validation FAILED. Some parameters are missing or invalid."
            )
        return is_valid
    ...
    def log_config_summary(self, log_level=logging.INFO):
        ...
        """
        if (
            not self._loaded and not self._env_file_loaded
        ):  # Check if any loading attempt was made
            logger.info(
                "No configuration loaded (YAML not found/empty and .env not processed). Summary not available."
            )
            return
        ...
        if self._env_file_loaded:
            ...
            logger.log(
                log_level,
                f"Source: .env file not found or not loaded from '{env_path_used}'.",
            )
        ...
        if self._loaded and self._config:  # _config has content from YAML
            logger.log(
                log_level, f"Source: YAML file loaded from '{yaml_config_path_used}'."
            )
        elif (
            self._loaded and not self._config
        ):  # YAML file was found but empty or parsed to None
            logger.log(
                log_level,
                f"Source: YAML file at '{yaml_config_path_used}' was empty or contained no data.",
            )
        else:  # YAML file not found (and it was the default path)
            logger.log(
                log_level, f"Source: YAML file not found at '{yaml_config_path_used}'."
            )

        logger.log(
            log_level,
            "Effective critical settings (priority: YAML > Environment > Default):",
        )
        for key_path, validation_rule in CRITICAL_CONFIG_KEYS:
            key_str = ".".join(key_path)
            value = self.get(
                key_path
            )  # This gets the effective value based on priority
            ...
            if value is not None:
                ...
                if (
                    ...
                    isinstance(value, bool) and not is_secret_key
                ):  # Handle booleans if added later
                    display_value = str(value)

                logger.log(log_level, f"  {key_str}: {display_value}")
            else:
                logger.log(
                    log_level, f"  {key_str}: Not set (and no default defined in get())"
                )
    ...
    def load(self):
        ...
        if os.path.exists(env_path):
            ...
            self._env_file_loaded = load_dotenv(
                env_path, verbose=True, override=True
            )  # override=True ensures .env can set vars even if they exist
            if self._env_file_loaded:
                logger.info(
                    "Loaded environment variables from '%s'. These may be overridden by environment-set variables.",
                    env_path,
                )
            else:  # load_dotenv returns False if file is empty or only comments
                logger.info(
                    ".env file at '%s' was processed but set no new environment variables (they may already exist or file is empty/comments-only).",
                    env_path,
                )
        else:
        ...
        if os.path.exists(config_path):
            try:
                ...
                logger.critical(
                    "Failed to load YAML configuration from '%s': %s", config_path, e
                )
            ...
            if config_path == config_path_default:
                logger.info(
                    "Default configuration file '%s' not found. Relying on environment variables and .env.",
                    config_path,
                )
                self._config = {}
                ...
                logger.critical(
                    "Specified configuration file '%s' not found. This is critical.",
                    config_path,
                )
                return False
    ...
    def get(self, key_tuple, default=None):
        ...
        try:
            ...
            if env_value is None:
                ...
                if env_value_enc:
                    logger.debug(
                        f"Found encrypted variable {env_key_enc}, attempting decryption."
                    )
            ...
            if env_value is not None:
                ...
                if expected_type == "int":
                    try:
                        ...
                        logger.warning(
                            "Env var '%s' with value '%s' could not be cast to int, returning as string. Validation will occur in validate_critical_config.",
                            env_key,
                            env_value,
                        )
                    ...
                    if env_value.lower() in ["true", "1", "yes", "y"]:
                        ...
                        logger.warning(
                            "Env var '%s' with value '%s' could not be cast to bool, returning as string. Validation will assess if this is acceptable.",
                            env_key,
                            env_value,
                        )
                        return env_value  # Return string, validation is centralized
